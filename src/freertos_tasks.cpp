...
void mainTask(void* pvParameters) {
  ...
  while (1) {
    ...
    // NOTE: Tests disabled - they cause stack overflow when run continuously
    // Tests should be run manually during development, not in production
...
void initializeFreeRTOS() {
  ...
  // extern DeviceManager deviceManager; // Removed global extern declaration

  // Initialize NetworkLogger (needs command queue to be created first)
  Serial.println("[FreeRTOS] Initializing NetworkLogger...");
  networkLogger.begin("/config.json", &timeManager, networkCommandQueue);
  ...
  // Create device manager task
  xReturned = xTaskCreate(
    deviceManagerTask,                     // Task function
    "DeviceMgrTask",                       // Task name
    TASK_STACK_DEVICE_MANAGER / 4,         // Stack size in words
    (void*)&DeviceManager::getInstance(),  // Parameter (DeviceManager instance)
    TASK_PRIORITY_DEVICE_MANAGER,          // Priority
    NULL                                   // Task handle
  );
  if (xReturned != pdPASS) {
    Serial.println("[FreeRTOS] ERROR: Failed to create Device Manager task!");
    return;
  }
  Serial.println("[FreeRTOS] Device Manager task created");

  // Create Cycle Manager task
  xReturned = xTaskCreate(
    cycleManagerTask,                      // Task function
    "CycleMgrTask",                        // Task name
    TASK_STACK_CYCLE_MANAGER / 4,          // Stack size in words
    NULL,                                  // Parameter
    TASK_PRIORITY_CYCLE_MANAGER,           // Priority
    NULL                                   // Task handle
  );
  if (xReturned != pdPASS) {
    Serial.println("[FreeRTOS] ERROR: Failed to create Cycle Manager task!");
    return;
  }
  Serial.println("[FreeRTOS] Cycle Manager task created");
...
void deviceManagerTask(void* pvParameters) {
  // Get reference to DeviceManager (passed as parameter)
  DeviceManager* pDeviceManager = (DeviceManager*)pvParameters;

  Serial.println("[FreeRTOS] Device Manager task started");
  ...
  while (1) {
    ...
    // Periodically read ADC values (if any configured)
    std::vector<DeviceInfo> allDevices = pDeviceManager->getAllDevices();
    for (const auto& device : allDevices) {
      if (strcmp(device.type, "adc") == 0) {
        int adc_value = pDeviceManager->getState(device.id);
        // Optionally log or use the ADC value for control logic
        // Logger::getInstance().logDebug("DeviceManagerTask: ADC device %s reading: %d", device.id, adc_value);
      }
    }
  }
}
...
void cycleManagerTask(void* pvParameters) {
  Serial.println("[FreeRTOS] Cycle Manager task started");
